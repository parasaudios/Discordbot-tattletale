...
| `/tattletale badword add word:<text> [channel:<#ch>] [notify:<@user/role…>] [wholeword:]` | Add a **bad word** (Judge-checked → tiered). Optional per-word channel + ping (one or more users/roles). |
| `/tattletale badword remove\|list\|clear` | Remove one / show all / clear all bad words. |
| `/tattletale goodword add word:<text> [channel:<#ch>] [notify:<@user/role…>] [wholeword:]` | Add a **good word** (safe, notify-only, **no Judge**). Optional per-word channel + ping (one or more users/roles). |
